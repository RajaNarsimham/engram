...
A LoRA trained on the facts recalls a substantial **69%** (far above the base's 0%): adapters genuinely *do* store facts, and the 0% of the §7.2 skill task—where the adapter was trained on a *disjoint* set—would understate them. But three findings keep retrieval indispensable. **(i) Recall is lossy and confabulatory:** the misses were not blanks but confident wrong answers, several distinct facts collapsing onto a single dominant memorized token (the adapter answered "Veddish" to four unrelated questions) and details corrupting ("tellurium"→"tellish")—failure modes retrieval does not exhibit, as its **100%** shows. **(ii) The adapter does not scale or update** like an index (§2). **(iii) Most strikingly, the memorization adapter *degrades* RAG:** Base + LoRA + RAG scored 69%, *below* RAG's 100%, because the fact-baked adapter partially overrides correct retrieved context (the P3 interference, here without the context-preserving fix). The lesson is sharp and counter-intuitive: consolidating *facts* into adapters not only fails to replace retrieval, it *harms* it. Facts belong in RAG; the adapter's value—if any—lies in *behaviors* RAG cannot supply, which §7.2 tests directly. (A methodological note we keep for honesty: an initial run of this experiment scored 6%, an artifact of a "say 'unknown' if unsure" instruction that induced abstention; removing it revealed the 69% above. We report it because it illustrates how easily a single prompt clause can mask a parametric model's true recall.)
