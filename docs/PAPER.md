...
The base scores zero on both, confirming the facts are genuinely novel. **RAG answers both seen and fresh** (90%/88%): retrieval *generalizes* to facts never trained on (P2). **A LoRA answers only seen facts** (58%/0%): fine-tuning cannot recall what it never trained on (P2). A *memorization-only* LoRA combined with RAG exhibited **interference**—fresh-fact accuracy collapsed (to 0% in an earlier run)—because the adapter taught the model to ignore retrieved context; training the adapter with **context-preserving** examples (P3) recovered fresh accuracy to 72% while retaining 82% on seen. This both reproduces the failure and validates the fix. Critically, **on pure fact recall RAG alone is optimal**: the fact-memorizing LoRA cannot help (RAG already supplies the verbatim fact), and even after the interference fix it *lowers* accuracy relative to RAG alone (90→82 seen, 88→72 fresh). The adapter's value—if any—must therefore lie in *behaviors* RAG cannot supply, which §7.2 tests directly.

**7.2 Does the adapter add value over RAG? A skill task.** Because §7.1 shows the adapter cannot help on facts, we test whether it helps on a *behavior* the base produces unreliably. The skill is *grounded answering in a strict template with abstention*: an answerable query must yield `ANSWER: <answer> | SOURCE: <entity>`, an unanswerable one exactly `ANSWER: NOT FOUND | SOURCE: NONE`. Both configurations receive the **same** system instruction and the **same** RAG context; the LoRA is trained only on the behavior, over a fact set *disjoint* from the evaluation, so it learns the convention, not the answers. Over 12 answerable and 12 unanswerable held-out questions:

| Configuration | format | answer | source | abstain |
|---|---:|---:|---:|---:|
| Base + RAG (instructed) | 100% | 100% | 0% | 100% |
| Base + LoRA + RAG | 100% | 92% | 92% | 100% |

We report the mixed result in full. On **format** and **abstention** the prompted base already scores 100%—*prompting suffices for these behaviors and the adapter adds nothing*. On **answer accuracy** the adapter is slightly *worse* (100→92%, one of twelve), a small residual cost consistent with §7.1's interference. The adapter's clear win is **source attribution, 0%→92%**: the instructed base never produced a clean canonical source—it wrote the literal word "Context" or echoed the whole retrieved sentence—whereas the adapter reliably emitted the correct entity. This is precisely the value a skill adapter should deliver: it *internalizes a precise convention that the instruction under-specifies*, making it the default behavior without prompt engineering. Honest caveats: a more elaborate prompt might narrow the source-attribution gap; the answer-accuracy regression is a real, if small, cost; and at n=24 these are indicative magnitudes, not precise estimates. The qualitative conclusion is robust and refines P2: **facts belong in retrieval; a skill adapter earns its place only for behaviors the base does unreliably—and not for free.**
...
- **Adapter value is narrow and not free.** §7.2 finds the skill adapter helps only on a behavior the prompted base does *unreliably* (canonical source attribution, 0→92%), adds nothing where prompting already suffices (format, abstention), and carries a small answer-accuracy cost (100→92%). We did *not* find a regime where an adapter broadly dominates retrieval-plus-prompting; characterizing which behavior classes justify an adapter—versus a better prompt—at scale is open. This tempers, with evidence, the intuition that more skills are always better.
